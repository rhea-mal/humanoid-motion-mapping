...
int main() {
	...
	static const string robot_file = "./resources/model/mmp_panda.urdf";
	...
	double start_hit_offset = 0.1;			// how far the EE should move during a swing
	double follow_through_scaling = -0.006;	// Scales time to continue swing past desired impact time
	double power_scaling = -8.0;
	double hit_height = 0.9;				// height at which to hit ball
	double min_time_to_reach_ball = 0.1;	// minimum time given for base to reach ball position
	double return_vel_scaling = -0.03;
	...
	while (runloop) {
		...
		if (state == HIT_BALL) {
			...
			Vector3d v1 = -ball_impact_vel*return_vel_scaling;
	...
	redis_client.setEigen(PANDA_JOINT_TORQUES_COMMANDED_KEY, 0 * command_torques);  // back to floating
